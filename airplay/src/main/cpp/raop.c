...
conn_request(void *ptr, http_request_t *request, http_response_t **response)
{
	...
	if (!require_auth && challenge) {
		...
		logger_log(conn->raop->logger, LOGGER_DEBUG, "conn->local:%d.%d.%d.%d", conn->local[0], conn->local[1],conn->local[2],conn->local[3]);
		logger_log(conn->raop->logger, LOGGER_DEBUG, "conn->locallen:%d", conn->locallen);

		logger_log(conn->raop->logger, LOGGER_DEBUG, "conn->hwaddr:%2x:%2x:%2x:%2x:%2x:%2x", \
			raop->hwaddr[0],raop->hwaddr[1],raop->hwaddr[2],raop->hwaddr[3],raop->hwaddr[4],raop->hwaddr[5]);
		logger_log(conn->raop->logger, LOGGER_DEBUG, "conn->hwaddrlen:%d", raop->hwaddrlen);
...
//				logger_log(conn->raop->logger, LOGGER_WARNING, "string:%s", datastr);
				if (!strncmp(datastr, "volume: ", 8)) {
                    ...
                    datastr[datalen - 1] = 0;
					raop_rtp_set_total_len(conn->raop_rtp, datastr + 10, datalen - 10 - 2);//elimate the \n character
//					logger_log(conn->raop->logger, LOGGER_WARNING, "string0:%s,datalen:%d:%d", datastr, datalen, strlen(datastr));
				}
