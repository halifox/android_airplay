...
struct httpd_s {
	...
	/* user control fd (pipe read from user cmd)*/
	/**
	 * [0] : read
	 * [1] : write
	 */
...
int send_close_connections_cmd(void)
{
	char usercmd = CMD_FROM_USER_CLOSE_CONNECTION;
	if (0 < pipe_write_fd){
		write(pipe_write_fd, &usercmd, sizeof(usercmd));
	}
...
httpd_thread(void *arg)
{
	...
	while (1) {
		...
		if (httpd->pipe_fds[0] > 0){
			FD_SET(httpd->pipe_fds[0], &rfds);
			if (nfds <= httpd->pipe_fds[0]) {
				nfds = httpd->pipe_fds[0]+1;
			}
		}
		...
		if (httpd->pipe_fds[0] > 0 && FD_ISSET(httpd->pipe_fds[0], &rfds)){
			ret = read(httpd->pipe_fds[0], &usrcmd, sizeof(usrcmd));
			if (0 > ret){
				logger_log(httpd->logger, LOGGER_INFO, "Error in reading from user cmd pipe");
				break;
			} else if (0 == ret){
				logger_log(httpd->logger, LOGGER_INFO, "the write cmd of the usr pipe cmd is closed");
				if (0 < httpd->pipe_fds[1]) close(httpd->pipe_fds[1]);
				if (0 < httpd->pipe_fds[0]) close(httpd->pipe_fds[0]);
				httpd->pipe_fds[0] = -1;
				httpd->pipe_fds[1] = -1;
				pipe_write_fd = -1;
				continue;
			...
			switch (usrcmd){
				case CMD_FROM_USER_CLOSE_CONNECTION :
					for (k = 0; k < httpd->open_connections; ++k) {
						if (httpd->connections[k].connected){
							httpd_remove_connection(httpd, &httpd->connections[k]);
						}
					}
					break;
				default:
					logger_log(httpd->logger, LOGGER_INFO, "invalid user cmd");
			}
...
//			memcpy(databuffer, buffer, ret);
//			ptemp = databuffer;
//			for(j = 0; j < ret; j++){
//				if(0x0d == databuffer[j]){
//					databuffer[j] = 0;
//					LOGE("data:%s", ptemp);
//					databuffer[j++] = 0;
//					ptemp = &databuffer[j + 1];
//				}
//			}

//			for(j = 0; j < ret; j++){
//				LOGE("%d:%c:%x", j, databuffer[j], databuffer[j]);
//			}
//
//			LOGE("this is a test");
...
httpd_start(httpd_t *httpd, unsigned short *port)
{
    ...
    /**
     * create a pipe for user control
     * fds[0] : for read data from pipe
     * fds[1] : for write data to pipe
     */
    ret = pipe(httpd->pipe_fds);
    if(0 > ret){
		logger_log(httpd->logger, LOGGER_ERR, "failed to create a pipe for user control");
		MUTEX_UNLOCK(httpd->run_mutex);
		return -1;
    }
	pipe_write_fd = httpd->pipe_fds[1];
...
//	}
	...
	on = 1;
	ret = setsockopt( httpd->server_fd4, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on) );
	if (0 > ret){
		closesocket(httpd->server_fd4);httpd->server_fd4 = -1;
		closesocket(httpd->server_fd6);httpd->server_fd6 = -1;
		close(httpd->pipe_fds[1]);httpd->pipe_fds[1] = -1;
		close(httpd->pipe_fds[0]);httpd->pipe_fds[0] = -1;
		pipe_write_fd = -1;
		MUTEX_UNLOCK(httpd->run_mutex);
		return -1;
	}
