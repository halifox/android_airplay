...
struct raop_rtp_s {
	...
	unsigned char *totallendata;
	int totallendata_len;
...
raop_rtp_destroy(raop_rtp_t *raop_rtp)
{
	if (raop_rtp) {
        ...
        free(raop_rtp->totallendata);
        raop_rtp->totallendata = NULL;
...
raop_rtp_resend_callback(void *opaque, unsigned short seqnum, unsigned short count)
{
	...
	logger_log(raop_rtp->logger, LOGGER_ERR, "resend request for %d %d", seqnum, count);
...
extern int pastpackets;
extern unsigned char timestamp[4];
...
raop_rtp_process_events(raop_rtp_t *raop_rtp, void *cb_data)
{
	...
	/* Read the totallen value */
	totallendata = raop_rtp->totallendata;
	totallendata_len = raop_rtp->totallendata_len;
	raop_rtp->totallendata = NULL;
	raop_rtp->totallendata_len = 0;
	...
	if (totallendata != NULL){
		if (raop_rtp->callbacks.audio_set_total_len) {
			raop_rtp->callbacks.audio_set_total_len(raop_rtp->callbacks.cls, cb_data, totallendata, totallendata_len);
		}
		free(totallendata);
		totallendata = NULL;
	}

	/* reach, just update the progress */
//	logger_log(raop_rtp->logger, LOGGER_WARNING, "pastpackets = %d", pastpackets);
	if (pastpackets >= PROGRESS_UPDATE_INTERVAL_PACKETS){
		if (raop_rtp->callbacks.audio_set_progress) {
			raop_rtp->callbacks.audio_set_progress(raop_rtp->callbacks.cls, cb_data, timestamp, 4);
		}
		pastpackets = 0;
	}
...
static int get_thread_policy( raop_rtp_t *raop_rtp, pthread_attr_t *attr)
{
	int policy;
	...
	{
		case SCHED_FIFO:
			logger_log(raop_rtp->logger, LOGGER_WARNING, "policy = SCHED_FIFO");
			break;

		case SCHED_RR:
			logger_log(raop_rtp->logger, LOGGER_WARNING, "policy = SCHED_RR");
			break;

		case SCHED_OTHER:
			logger_log(raop_rtp->logger, LOGGER_WARNING, "policy = SCHED_OTHER");

			break;

		default:
			logger_log(raop_rtp->logger, LOGGER_WARNING, "policy = UNKNOWN");
			break;
	}
...
static void show_thread_priority( raop_rtp_t *raop_rtp, pthread_attr_t *attr, int policy )
{
	int priority = sched_get_priority_max( policy );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "max_priority = %d", priority);


	priority = sched_get_priority_min( policy );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "min_priority = %d", priority);
}

static int get_thread_priority( raop_rtp_t *raop_rtp, pthread_attr_t *attr )
{
	struct sched_param param;

	int rs = pthread_attr_getschedparam( attr, &param );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "priority = %d", param.sched_priority);

	return param.sched_priority;
}

static int set_thread_priority( raop_rtp_t *raop_rtp, pthread_attr_t *attr )
{
	struct sched_param param;
...
static void set_thread_policy( raop_rtp_t *raop_rtp, pthread_attr_t *attr,  int policy )
{
	int rs = pthread_attr_setschedpolicy( attr, policy );

	get_thread_policy( raop_rtp, attr );
}


#define CMD_FROM_USER_CLOSE_CONNECTION	0x01
#define DISCONNECT_TIME_OUT		(6 * 1000 / 5)
...
raop_rtp_thread_udp(void *arg)
{
	...
	int disconnecttime;
	int usercmdSendflag = 0;
	assert(raop_rtp);

	rs = pthread_attr_init( &attr );
	assert( rs == 0 );
	policy = get_thread_policy(raop_rtp , &attr );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "Show current configuration of priority");
	show_thread_priority( raop_rtp, &attr, policy );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "Show SCHED_FIFO configuration of priority");
	show_thread_priority( raop_rtp, &attr, SCHED_FIFO );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "Show SCHED_RR configuration of priority");
	show_thread_priority( raop_rtp, &attr, SCHED_RR );
	logger_log(raop_rtp->logger, LOGGER_WARNING, "Show priority of current thread");
	priority = get_thread_priority( raop_rtp, &attr );

	set_thread_policy( raop_rtp, &attr, SCHED_FIFO );
...
//		logger_log(raop_rtp->logger, LOGGER_ERR, "disconnecttime %d", disconnecttime);
		if (ret == 0) {
			/* Timeout happened */
			disconnecttime++;
			/**
			 * send only once
			 */
			if (disconnecttime > DISCONNECT_TIME_OUT && !usercmdSendflag){
			...
			printf("FIXME: Error happened:errno:\n", errno);
			break;
		}
...
//				logger_log(raop_rtp->logger, LOGGER_DEBUG, "Got control packet of type 0x%02x", type);
				...
				if (type == 0x56) {
					...
					seqnum = (packet[4 + 2] << 8) | packet[4 + 3];
					logger_log(raop_rtp->logger, LOGGER_WARNING, "Got retramsmit packet seq : %d", seqnum);
					assert(ret >= 0);
				}
			}
...
void
raop_rtp_set_total_len(raop_rtp_t *raop_rtp, const char *data, int datalen)
{
	...
	totallen = malloc(datalen);
	assert(totallen);
	memcpy(totallen, data, datalen);

	/* Set metadata in thread instead */
	MUTEX_LOCK(raop_rtp->run_mutex);
	raop_rtp->totallendata = totallen;
	raop_rtp->totallendata_len = datalen;
	MUTEX_UNLOCK(raop_rtp->run_mutex);
}

//void
//raop_rtp_set_progress(raop_rtp_t *raop_rtp, const char *data, int datalen)
//{
...
//		return;
//	}
//	progress = malloc(datalen);
//	assert(progress);
//	memcpy(progress, data, datalen);
//
//	/* Set metadata in thread instead */
//	MUTEX_LOCK(raop_rtp->run_mutex);
//	raop_rtp->progressdata = progress;
//	raop_rtp->progressdata_len = datalen;
//	MUTEX_UNLOCK(raop_rtp->run_mutex);
//}
